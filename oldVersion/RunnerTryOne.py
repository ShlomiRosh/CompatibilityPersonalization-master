...
class Runner:
    ...
    def __run_experiment(self, params):
        ...
        if not params['autotune_hyperparams']:  # dont evaluate hyper-param
            loop_modes = [False]
        self.__outer_folds_loop(params, timer_evaluating_params, timer_validation_results, timer_test_results, timers,
                                loop_modes, iterations, params_list)
        self.__show_result(params)

        print('\ndone')

    def __outer_folds_loop(self, params, timer_evaluating_params, timer_validation_results, timer_test_results, timers,
                           loop_modes, iterations, params_list):
        ...
        for seed_idx, seed in enumerate(params['seeds']):
            ...
            self.__inner_folds_loop(params, loop_modes, done_by_inner_seed, timer_validation_results,
                                    hists_seed_by_user, timer_evaluating_params, seed, timers, iterations,
                                    timer_test_results, h2_train_and_valid, hist_train_and_valid_ranges,
                                    h2_train_and_valid_x,
                                    h2_train_and_valid_y, seed_idx, params_list)

    def __inner_folds_loop(self, params, loop_modes, done_by_inner_seed, timer_validation_results, hists_seed_by_user,
                           timer_evaluating_params, seed, timers, iterations, timer_test_results, h2_train_and_valid,
                           hist_train_and_valid_ranges, h2_train_and_valid_x,
                           h2_train_and_valid_y, seed_idx, params_list):
        ...
        for evaluating_params in loop_modes:
            ...
            for inner_seed_idx, inner_seed in enumerate(params['inner_seeds']):
                ...
                user_count = 0
                self.__user_loop(params, hists_inner_seed_by_user, h2_train, user_count, evaluating_params,
                                 done_last_users,
                                 timer_validation_results, timer_evaluating_params, h2_train_x, h2_train_y,
                                 scores_per_user,
                                 best_params_per_user, h1, seed, inner_seed, timers, iterations, hists_seed_by_user,
                                 timer_test_results, h2_train_and_valid,
                                 hist_train_and_valid_ranges, h2_train_and_valid_x,
                                 h2_train_and_valid_y, seed_idx, inner_seed_idx, params_list)

        self.__test_models(params, hists_seed_by_user, timer_test_results, h2_train_and_valid,
                           hist_train_and_valid_ranges, best_params_per_user, h1, h2_train_and_valid_x,
                           h2_train_and_valid_y, seed, inner_seed, timers, iterations, seed_idx, inner_seed_idx,
                           params_list)

    def __user_loop(self, params, hists_inner_seed_by_user, h2_train, user_count, evaluating_params, done_last_users,
                    timer_validation_results, timer_evaluating_params, h2_train_x, h2_train_y, scores_per_user,
                    best_params_per_user, h1, seed, inner_seed, timers, iterations, hists_seed_by_user,
                    timer_test_results, h2_train_and_valid,
                    hist_train_and_valid_ranges, h2_train_and_valid_x,
                    h2_train_and_valid_y, seed_idx, inner_seed_idx, params_list):
        ...
        for user_id, item in hists_inner_seed_by_user.items():
            ...
            self.__log_progress(runtime, mod_str, params, timers, iterations, seed_idx, inner_seed_idx, user_count)
        # end user loop
        # end inner folds loop
        # end train and validation loop

    def __test_models(self, params, hists_seed_by_user, timer_test_results, h2_train_and_valid,
                      hist_train_and_valid_ranges, best_params_per_user, h1, h2_train_and_valid_x,
                      h2_train_and_valid_y, seed, inner_seed, timers, iterations, seed_idx, inner_seed_idx,
                      params_list):
    ...
    def __log_progress(self, runtime, mod_str, params, timers, iterations, seed_idx, inner_seed_idx, user_count,
                       verbose=True):
